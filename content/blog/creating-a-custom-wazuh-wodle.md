...
tags = ['open-source', 'security-engineering', 'security-monitoring', 'siem', 'wazuh']
+++
...
In `ossec.log` you'll get system-level information, including wodle execution:

```bash
2025/03/02 13:49:05 wazuh-modulesd:command: INFO: Starting command 'hello-world'.
```

While `alerts.log` will show the alerts generated by the custom rule:
...
And that's it! You've successfully created a custom wodle with an associated rule that extends Wazuh's monitoring capabilities to include your custom data source.
